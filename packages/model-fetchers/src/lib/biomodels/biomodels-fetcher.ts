...
export class BiomodelsModelFetcher implements ModelFetcher {
  ...
  async fetchModel(modelId: string): Promise<FetchedModel> {
    ...
    if (!filename) {
      throw new Error(
        `BioModels model ${modelId} does not expose files.main.name`,
      );
    }

    const downloadedFile = await this.apiClient.downloadModelFile(
      modelId,
      filename,
    );
  ...
  getModelSource(modelId: string): string | undefined {
    return `https://www.biomodels.org/${encodeURIComponent(modelId)}`;
  }
}
